        ...
        @foreach(ViewDataHelper::getLanguages() as $language)
            <div class="tab-pane @if($loop->first) active @endif"
                ...
                <div class="form-group required @if($errors->has("locales[{$language->code}][title]")) has-error @endif">
                    ...
                    {{ Form::label("locales[{$language->code}][title]", 'Category Title', ['class' => 'col-md-2 control-label']) }}

                    <div class="col-md-10">
                        {{ Form::text("locales[{$language->code}][title]", $category->getLocaled('title', $language->code), ['class' => 'form-control', 'placeholder' => 'Name', 'required']) }}
